...
Para obtener conexiones entre servidores de costo mínimo y de replicación mínima, el master del AGM debe ser el master del grafo original. Si el master del grafo original no es el mismo que el del AGM y existe un único AGM, entonces no hay otra manera de arreglar el problema, ya que es necesario utilizar las aristas de peso mínimo. Si hay más de un AGM, se puede calcular el nodo master para cada uno de ellos y ver si coincide con el nodo master del grafo original, si esto pasa, habremos encontrado un AGM con replicación óptima.

\vspace{1em}

\textbf{¿Cómo se debe modificar la solución si en lugar de transmitir por broadcast se lo hace por
multicast, es decir, se debe mandar un paquete a cada destino, sin hacer copias?}

Sea $u$ un nodo cualquiera. El costo monetario de replicación del contenido de $u$ a otro nodo cualquiera $v$ es igual al costo del camino entre $u$ y $v$ por el que se envió la copia. Es decir que para minimizar el costo monetario de enviar una copia desde $u$ a $v$ basta con hallar el camino mínimo entre ambos.

El proceso de replicación por multicast no es más que repetir el proceso anterior secuencialmente para todo vértice $v$ distinto de $u$. Entonces el costo monetario mínimo de replicar el contenido por multicast desde el vértice $u$ al resto de los nodos está dado por la suma de los costos de los caminos mínimos entre $u$ y cada $v$ distinto de $u$.

Entonces para minimizar globalmente el costo de replicar el contenido por multicast, basta con hallar la suma de los caminos mínimos entre el nodo $u$ y el resto de los nodos del grafo, para todo $u$, y quedarnos con el nodo $u$ que minimiza este costo. Por ejemplo, podemos ejecutar el algoritmo de Dijkstra tomando como nodo origen cada nodo del grafo y quedarnos con el nodo que minimiza la suma de todos los caminos mínimos entre sí mismo y el resto de los nodos del grafo.

Una vez que hallamos este nodo, podemos descartar los enlaces que no son usados por ningún camino mínimo.
...
La complejidad temporal del \textbf{peor caso} deberá ser $O(C^2)$ o bien $O(R \times log \, C)$.
...
% Creamos un nodo fantasma que conecta todas las fábricas a través de rutas de costo cero. Luego obtenemos un árbol generador mínimo por medio del algoritmo de Kruskal. Finalmente quitamos el nodo fantasma del árbol generador mínimo obtenido junto con las rutas asociadas y devolvemos el bosque resultante.
...
\begin{pseudo}
  \Procedure{Transportes-Pesados}{$f,\; G=(V, E)$}
    \State $v \leftarrow |V| + 1$ \Comment{Creamos un nodo fantasma}
    \State $V \leftarrow V \cup \{ v \}$ \Comment{Agregamos el nodo fantasma al grafo}
    \For{$i \leftarrow$ \To $f$}                                                         \Ode{|V|}
      \State $E \leftarrow E \cup \{ (i, v, 0)  \}$ \Comment{Conectamos cada fábrica al nodo fantasma con costo 0}
    \EndFor
    \State $E' \leftarrow \textsc{Kruskal}(G)$                                           \Ode{|E| \times log\, |V|}
...
% Se desea hallar el subconjunto de rutas con costo mínimo tal que todos los clientes tienen una ruta hacia una fábrica.

% Lo que se nos pide es un bosque de árboles que no necesariamente son generadores, pues un conjunto de clientes conectados entre sí se satisface con una sola fábrica, y en caso de haber más de una fábrica conectada únicamente a este conjunto de clientes, es correcto quedarnos con la fábrica cuya ruta al grafo de clientes es mínima y desconectar el resto de las fábricas.

% Para simplificar el problema, introducimos un nodo fantasma conectado a todas las fábricas por medio de rutas de costo cero.

% Lo que esto logra es que al correr el algoritmo de Kruskal sobre el grafo resultante, los nodos del grafo van conectarse entre sí uno a la vez, siguiendo una estructura de árbol, consumiendo las rutas por orden de costo creciente. Esto quiere decir que primero se van a conectar todas las fábricas entre sí, formando un árbol de fábricas unidas por el nodo fantasma, pues las rutas de este nodo son las de menor costo (costo cero). Luego se van a unir los clientes entre sí y con las fábricas, conectándose siempre primero por las rutas más baratas, hasta que todos los nodos del grafo estén unidos o hasta haber agotado las rutas.

% Es cierto que un árbol de clientes queda unido al árbol de fábricas por medio del 
