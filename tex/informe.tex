...
\end{pseudo}
\subsection{Complejidad}
Para la implementación del algoritmo base de Kruskal, se utilizó la estructura Disjoint-Set, compuesta por las funciones Make\_union() y Find() y 3 arreglos de longitud cantidad de nodos.
...
Como las operaciones sobre la estructura Disjoint-Set se realizan R veces, se obtiene una complejidad de O(R) para conseguir los subconjuntos de rutas que unen clientes con fábricas.

Debido a que el ordenamiento de las rutas, utilizamos la función Sort() de la STL, se obtiene una costo de O($R * log R$), lo que equivale a O($R * log(C^2)$) por ser $C^2$ la cantidad máxima de aristas lo que equivale a O($R * 2 log(C)$) = O($R * log(C)$).
