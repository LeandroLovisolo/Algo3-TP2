...
\end{pseudo}
\subsection{Complejidad}
Debido a que para el ordenamiento de las rutas, utilizamos la función Sort() de la STL, se obtiene una costo de O(R * log R), siendo R rutas, lo que equivale a O(R * log($(C+F)^2$)), siendo C clientes y F fábricas, por ser $(C+F)^2$ la cantidad máxima de aristas lo que equivale a O(R * 2 log(C+F)) = O(R * log(C+F)).
...
Como O(R) $\subset$ O(R * log (C+F)), la complejidad final del algoritmo es O($R * log C+F$)
\subsection{Correctitud}
