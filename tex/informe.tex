...
\begin{pseudo}
\Procedure{Impresiones-ordenadas}{n: entero, costos: matriz}
    \For{int j = n - 1; j $>$= 0; j$--$}
        \For{int i = 0; i $<$ n; i$++$}
            \If{i == 0 $\&\&$ j == 0}                 \Ode{1}
...
\subsection{Solución}
Dada una serie de aristas y costos, que representan las rutas entre clientes y fábricas y el costo de pavimentación respectivamente, utilizamos para la resolución una modificación del conjuto de datos para luego aplicar el algoritmo de Kruskal con la estrucuta Disjoint-Set.

La idea de la utilización del algoritmo de Kruskal nace de la necesidad de conseguir un bosque de AGM's entre clientes y fábricas. Como en nuestro caso, solo se necesita que una fábrica este conectada por una serie de rutas a otros clientes, creamos un nodo \textit{fantasma} al cuál le conectamos cada fábrica mediante una arista de costo cero para poder tener un único set que forzará a que el algoritmo solo conecte una única vez una fábrica con un conjunto.

El resultado obtenido será un árbol AGM al cuál, al sacarle el nodo \textit{fantasma} y las aristas de las fábricas a este, se desvincula las fábricas del nodo fantasma formando un bósque de AGM's que tendrá las rutas necesarias para pavimentar y minimizar el costo.
